...
void loop() {
  ...
  if (packetSize) {
    ...
    //check if the packetBuffer is equal to a desired string.
    //perform different commands depending on the string that was received.
...
//    Udp.endPacket();
    ...
    if (Udp.beginPacket(Udp.remoteIP(), Udp.remotePort()) == 0) {
      Serial.println ("fuck!");
    }
    else if (Udp.beginPacket(Udp.remoteIP(), Udp.remotePort()) == 1) {
      Serial.println ("yay!");
